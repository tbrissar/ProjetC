...
{
  switch(cont){
    case bombe : return (modeaffichage == 'D') ? 'o' : ' ';
    //if(modeaffichage == 'D'){return('o');}else{return(' ');}
                break;
    case vide : return ' ';
                break;
    case trou : return 'X';
...
void display(cellule **plateau, int tour, int N, int nbjoueurs, joueur *tabjoueurs)
{
  ...
  for(int k = 0; k < N; k++){
    sprintf(affiche,"%s%2d ", affiche, k);
  }
...
int checkcapture(cellule **plateau, int x, int y, direction dir, color coul, int N)
{
  ...
  if(isEdge(i, j, dir, N)) return 0 ;
  else{
    nextCell = plateau[i + dir.hori][j + dir.verti];
    if(nextCell.contenu != pion || nextCell.couleur == coul) return 0;
  }
  ...
  while(nextCell.couleur != coul){
    ...
    if(isEdge(i, j, dir, N)) return 0;
      ...
      nextCell = plateau[i + dir.hori][j + dir.verti];
      if(nextCell.contenu != pion) return 0;
    }
  }

  return ((i - x) != 0) ? abs(x - i) : abs(y - j);
}
...
void capture(cellule **plateau, fleche *rose, int x, int y, color coul)
{
  ...
  for(int i = 0; i < 8; i++){
    ...
    for(int j = 0; j <= rose[i].nbcases; j++){
      cellule *cell = &plateau[x + dir1 * j][y + dir2 * j];
      cell -> couleur = coul;
      cell -> contenu = pion;
      cell -> age = 1;
    }
...
int pose(cellule **plateau, fleche *rose, joueur j, int tour, int N, int nbjoueurs, joueur *tabjoueurs)
{
  ...
  if(j.ordi == 1){
    ...
    while(s == 0){
      ...
      if(s == 0) sendMessage(j.sockfd, instrposepasok);
    }
  ...
  //system(sleepslow);
  sendToAll(instrclear, nbjoueurs, tabjoueurs);
  if(plateau[x][y].contenu == bombe) explosion(plateau, coul, rose, x, y, N, nbjoueurs, tabjoueurs);
  else capture(plateau, rose, x, y, coul);
...
{
  direction dirinverse = {-dir.hori,-dir.verti};
  return dirinverse;
}
...
int checkfin(cellule **plateau, fleche *rose, joueur *tabjoueurs, int cpt, int N, int nbjoueurs)
{
  for(int k = cpt; k < nbjoueurs; k++,(cpt++)%2){
    if(verifcouprestant(plateau, rose, tabjoueurs[k],N).coordx!=-1) return k;
  }
  return -1;
}
...
void scores(cellule **plateau, joueur *tabjoueurs, int N, int nbjoueurs)
{
  ...
  for(int k = 0; k < nbjoueurs; k++){
    ...
    for(int i = 0; i < N; i++){
      for(int j = 0; j < N; j++){
        cell = plateau[i][j];
        if(cell.contenu == pion && cell.couleur == coul) score++;
      }
    }
    sendToAll(instrtext, nbjoueurs, tabjoueurs);
